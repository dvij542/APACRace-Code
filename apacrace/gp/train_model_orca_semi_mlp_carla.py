...
from apacrace.models import Kinematic6, Dynamic
import random
from apacrace.utils.plots import plot_true_predicted_variance
import torch
...
class DynamicModel(torch.nn.Module):
	def __init__(self, model, deltat = 0.01):
		...
		self.Ry[0].bias.data = -torch.arange(0.,0.6,(.6)/12.).to(torch.float64)
		...
		self.Fy[0].bias.data = -torch.arange(0.,0.6,(.6)/12.).to(torch.float64)
	...
	def forward(self, x, debug=False):
		...
		for i in range(2) :
			...
			# 	+ (rpms>=1700)*(550)
			a_pred = self.model.Cm1*pwm*(3.45*0.919)/(0.34*1265) \
				+ self.model.Cm1_brake*brakes*(3.45*0.919)/(0.34*1265)
...
def load_data(CTYPE, TRACK_NAME, VARIDX, xscaler=None, yscaler=None):
	data_dyn = np.loadtxt('../mpc/run0_data.csv',delimiter=',').T
	# print(y_all)
	yaw = data_dyn[6,3:-3]
	vx = data_dyn[7,3:-3]*np.cos(yaw) + data_dyn[8,3:-3]*np.sin(yaw)
	vy = -data_dyn[7,3:-3]*np.sin(yaw) + data_dyn[8,3:-3]*np.cos(yaw)
	data_dyn[7,3:-3] = vx
	data_dyn[8,3:-3] = vy
	y_all = (data_dyn[4:10,6:]-data_dyn[4:10,:-6])/6. #- data_kin['states'][:6,1:N_SAMPLES+1]
	x = np.concatenate([
		data_dyn[-3:-1,3:-3].T,
		data_dyn[-1,3:-3].reshape(1,-1).T,
		data_dyn[7:10,3:-3].T,
		data_dyn[4:6,3:-3].T],
		axis=1)
...
vx = x_train[:,3]
steer = x_train[:,1]
plt.plot(np.arctan2((w*vehicle_model.lr-vy),vx),label='alpha_r')
plt.plot(steer - np.arctan2((w*vehicle_model.lf+vy),vx),label='alpha_f')
...
for i in range(10000) :
	...
	if i==4999 :
		outputs = model(x_train[30:-30],debug=False)
	else :
		outputs = model(x_train[30:-30])
	# print(y_train[30:].shape[0])
	# model.Fy.requires_grad_ = False
	

	loss = loss_fn(outputs[:,1:], y_train[30:-30,1:])
	loss.backward()
